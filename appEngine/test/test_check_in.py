...
class SimplisticClass(unittest.TestCase):
    ...
    def test_check_in_success(self):
        checkin = check_in(datetime.now())
        self.assertTrue(checkin)

    def test_check_in_out_of_range(self):
        d = date(2005, 7, 14)
        t = time(6, 30)
        self.assertRaises(Exception, check_in, datetime.combine(d, t))

    def test_check_in_with_notification(self):
        d = date(2005, 7, 14)
        t = time(9, 01)
        self.assertEqual(check_in(datetime.combine(d, t)), "Check_in after 9am. A report will be generated")
